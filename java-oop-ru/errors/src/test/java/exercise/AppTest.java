...
class AppTest {
    ...
    void testPrintSquare() throws Exception {
        ...
        assertThat(result.trim()).isEqualTo("79" + System.lineSeparator() + "Вычисление окончено");

        Circle circle1 = new Circle(new Point(2, 3), -5);
        String result1 = tapSystemOut(() -> {
            App.printSquare(circle1);
        });
        assertThat(result1.trim()).isEqualTo("Не удалось посчитать площадь" + System.lineSeparator() + "Вычисление окончено");
    }
}
